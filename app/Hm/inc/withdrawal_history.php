...
$id = $userinfo['id'];
  ...
  $typewhere = ' and type in (\'withdrawal\', \'withdraw_pending\')';
  ...
  while ($row = mysql_fetch_array($sth)) {
      ...
      $row['actual_amount'] = number_format(0 - $row['actual_amount'], 2);
  ...
  $q = 'select sum(actual_amount) as periodsum from history where '.$datewhere.' and user_id = '.$id.' and type in (\'withdrawal\', \'withdraw_pending\')';
  $sth = db_query($q);
  $row = mysql_fetch_array($sth);
  $periodsum = $row['periodsum'];
  view_assign('periodsum', number_format(0 - $periodsum, 2));
  $q = 'select sum(actual_amount) as sum from history where 1=1 '.$userwhere.' and user_id = '.$id.' and type in (\'withdrawal\', \'withdraw_pending\')';
  $sth = db_query($q);
  $row = mysql_fetch_array($sth);
  $allsum = $row['sum'];
  ...
  view_execute('withdrawal_history.blade.php');
