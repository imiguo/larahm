...
$stats = [];
  ...
  foreach (app('data')->exchange_systems as $id => $data) {
      if ($data['status'] == 1) {
          $q = 'select sum(actual_amount) as col from history where type=\'add_funds\' and to_days(now()) = to_days(date) and ec='.$id;
          $sth = db_query($q);
          $row = mysql_fetch_array($sth);
          $in[$id]['today'] = abs($row['col']);
          $in['total']['today'] += abs($row['col']);
          $q = 'select sum(actual_amount) as col from history where type=\'withdrawal\' and to_days(now()) = to_days(date) and ec='.$id;
          $sth = db_query($q);
          $row = mysql_fetch_array($sth);
          $out[$id]['today'] = abs($row['col']);
          $out['total']['today'] += abs($row['col']);
          $q = 'select sum(actual_amount) as col from history where type=\'add_funds\' and yearweek(now()) = yearweek(date) and ec='.$id;
          $sth = db_query($q);
          $row = mysql_fetch_array($sth);
          $in[$id]['week'] = abs($row['col']);
          $in['total']['week'] += abs($row['col']);
          $q = 'select sum(actual_amount) as col from history where type=\'withdrawal\' and yearweek(now()) = yearweek(date) and ec='.$id;
          $sth = db_query($q);
          $row = mysql_fetch_array($sth);
          $out[$id]['week'] = abs($row['col']);
          $out['total']['week'] += abs($row['col']);
          $q = 'select sum(actual_amount) as col from history where type=\'add_funds\' and EXTRACT(YEAR_MONTH FROM now()) = EXTRACT(YEAR_MONTH FROM date) and ec='.$id;
          $sth = db_query($q);
          $row = mysql_fetch_array($sth);
          $in[$id]['month'] = abs($row['col']);
          $in['total']['month'] += abs($row['col']);
          $q = 'select sum(actual_amount) as col from history where type=\'withdrawal\' and EXTRACT(YEAR_MONTH FROM now()) = EXTRACT(YEAR_MONTH FROM date) and ec='.$id;
          $sth = db_query($q);
          $row = mysql_fetch_array($sth);
          $out[$id]['month'] = abs($row['col']);
          $out['total']['month'] += abs($row['col']);
          $q = 'select sum(actual_amount) as col from history where type=\'add_funds\' and year(now()) = year(date) and ec='.$id;
          $sth = db_query($q);
          $row = mysql_fetch_array($sth);
          $in[$id]['year'] = abs($row['col']);
          $in['total']['year'] += abs($row['col']);
          $q = 'select sum(actual_amount) as col from history where type=\'withdrawal\' and year(now()) = year(date) and ec='.$id;
          $sth = db_query($q);
          $row = mysql_fetch_array($sth);
          $out[$id]['year'] = abs($row['col']);
          $out['total']['year'] += abs($row['col']);
          $q = 'select sum(actual_amount) as col from history where type=\'add_funds\' and ec='.$id;
          $sth = db_query($q);
          $row = mysql_fetch_array($sth);
          $in[$id]['total'] = abs($row['col']);
          $in['total']['total'] += abs($row['col']);
          $q = 'select sum(actual_amount) as col from history where type=\'withdrawal\' and ec='.$id;
          $sth = db_query($q);
          $row = mysql_fetch_array($sth);
  ...
  $q = 'select count(*) as col from users where id <> 1 and status = \'on\'';
  $sth = db_query($q);
  $row = mysql_fetch_array($sth);
  $members_active = $row['col'];
  $q = 'select sum(status = \'on\') as col1, count(*) as col2 from types';
  $sth = db_query($q);
  $row = mysql_fetch_array($sth);
  $types_active = $row['col1'];
  $types_all = $row['col2'];
  $q = 'select sum(actual_amount) as col from history';
  $sth = db_query($q);
  $row = mysql_fetch_array($sth);
  $total_amount = $row['col'];
  $q = 'select sum(actual_amount) as col from deposits';
  $sth = db_query($q);
  $row = mysql_fetch_array($sth);
  $total_deposits = $row['col'];
  $q = 'select sum(actual_amount) as col from deposits where status=\'on\'';
  $sth = db_query($q);
  $row = mysql_fetch_array($sth);
  ...
  $q = 'select sum(actual_amount) as col from history where type=\'withdraw_pending\'';
  $sth = db_query($q);
  $row = mysql_fetch_array($sth);
  $pending_withdraw = abs($row['col']);
  $q = 'select sum(actual_amount) as col from history where type=\'commissions\'';
  $sth = db_query($q);
  $row = mysql_fetch_array($sth);
